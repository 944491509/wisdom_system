  ...
  new Vue({
    ...
    methods: {
      ...
      edit(id) {
        axios.post("/api/notice/notice-info", { notice_id: id }).then(res => {
          if (Util.isAjaxResOk(res)) {
            ...
            o.organization = o.organization.map(e => {
              return { organization_id: e.id, name: e.name };
            });
            o.grade = o.grade.map(e => {
              return { grade_id: e.id, name: e.name };
            });
      ...
      loadNotice: function(id) {
        this.isLoading = true;
        axios.post("/school_manager/notice/load", { id: id }).then(res => {
          if (Util.isAjaxResOk(res)) {
      ...
      onSubmit: function() {
        ...
        this.isLoading = true;
        axios
          .post("/school_manager/notice/save-notice", { notice: this.notice })
          .then(res => {
            if (Util.isAjaxResOk(res)) {
      ...
      newNotice: function() {
        axios
          .post("/school_manager/notice/save-notice", { onlyacl: 1 })
          .then(res => {
            if (res.data == "ok") {
              this.releaseDrawer = true;
              this.$nextTick(() => {
      ...
      deleteNoticeMedia: function(id) {
        this.isLoading = true;
        axios
          .post("/school_manager/notice/delete-media", { id: id })
          .then(res => {
            if (Util.isAjaxResOk(res)) {

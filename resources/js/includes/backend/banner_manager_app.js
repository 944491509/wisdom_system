  ...
  if (document.getElementById("banner-manager-app")) {
    new Vue({
      ...
      methods: {
        ...
        getTypeList() {
          axios
            .post(Constants.API.BANNER.POST_TYPE, {})
            .then(res => {
        ...
        async editBanner(id) {
          let onlyacl = await axios.post(
            Constants.API.BANNER.POST_SAVE_BANNER,
            {
              onlyacl: 1
            }
          );
          ...
          _that_.getTypeList();
          axios
            .post(Constants.API.BANNER.GET_BANNER_ONE, { id: id })
            .then(res => {
              if (Util.isAjaxResOk(res)) {
                // 图片回显
                let obj = {};
                ...
                _that_.bannerFormInfo.public =
                  res.data.data.public == 1 ? true : false;
                _that_.bannerFormInfo.status =
                  res.data.data.status == 1 ? true : false;
                console.log("===================修改===================");
                console.log(_that_.bannerFormInfo);
                _that_.handlechange({
                  0: res.data.data.app,
                  1: res.data.data.posit,
                  2: res.data.data.type
                });
              }
            })
            .catch(err => {
        ...
        handlechange(value, type = 1) {
          ...
          if (bannerFormInfoType > 0) {
            if (bannerFormInfoType == 1) {
              // 1 => '无跳转'
              this.isshow1 = false;
              this.isshow2 = false;
            }
            if (bannerFormInfoType == 2) {
              // 2 => '图文'
              this.isshow1 = false;
              this.isshow2 = true;
              console.log("--------图文----");
            ...
            if (bannerFormInfoType == 4) {
              // 4 => '校园网'
              this.isshow1 = false;
              this.isshow2 = false;
            }
            if (bannerFormInfoType == 5) {
              // 5 => '招生主页'
              this.isshow1 = false;
              this.isshow2 = false;
            }
            if (bannerFormInfoType == 6) {
              // 6 => '迎新主页'
              this.isshow1 = false;
              this.isshow2 = false;
            }
            if (bannerFormInfoType == 7) {
              // 7 => '选课列表页'
              this.isshow1 = false;
              this.isshow2 = false;
            }
            if (bannerFormInfoType == 8) {
              // 8 => '社区动态详情'
              this.isshow1 = true;
              this.isshow1Lable = "动态";
              this.isshow1Placeholder = "例如：800";
            }
            if (bannerFormInfoType == 11) {
              // 1 => '无跳转'
              this.isshow1 = false;
              this.isshow2 = false;
            }
            if (bannerFormInfoType == 12) {
              // 2 => '图文'
              this.isshow1 = false;
              this.isshow2 = true;
            }
            if (bannerFormInfoType == 13) {
              // 3 => 'URL'
              this.isshow1 = true;
              this.isshow1Lable = "URL";
              this.isshow1Placeholder = "URL格式为：http://app.pftytx.com/";
        ...
        saveBannerInfo: function() {
          ...
          axios
            .post(Constants.API.BANNER.POST_SAVE_BANNER, params)
            .then(res => {
              if (Util.isAjaxResOk(res)) {
                this.$message({
                  type: "success",
        ...
        deleteBanner: async function(id) {
          let onlyacl = await axios.get(
            "/school_manager/banner/delete?onlyacl=1"
          );
          if (onlyacl.data != "ok") return;

  ...
  <div class="courses-manager-wrap">
      ...
      >
        ...
        <el-form-item label="所属专业" prop="majors">
          <el-select
            ...
            multiple
            placeholder="请选择所属专业. 留空表示对所有的专业都有效"
            style="width: 100%;"
          >
            <el-option
    ...
    >
      <el-form :model="attachForm" ref="attachTextbooksForm">
        <el-form-item label="所属专业" prop="majors">
          <el-select
            v-model="attachForm.attachedTextbooks"
            multiple
            placeholder="请选择教材"
            style="width: 90%;"
          >
            <el-option v-for="(tb, idx) in textbooks" :key="idx" :label="tb.name" :value="tb.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item>
    ...
    <el-drawer title="课程表" :visible.sync="showCourseScheduleFlag" direction="rtl" size="50%">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        teachers: [{ required: true, message: "授课教师", trigger: "change" }]
      },
  ...
  methods: {
    ...
    attachTextbookConfirmed: function() {
      attachTextbooksToCourse(
        this.schoolId,
        this.attachForm.courseId,
        this.attachForm.attachedTextbooks
      ).then(res => {
        if (Util.isAjaxResOk(res)) {
          // 关联成功
    ...
    onCourseNeedDelete: function(payload) {
      this.$confirm("此操作将永久删除该课程, 是否继续?", "提示", {
        ...
        .then(() => {
          axios
            .post(Constants.API.DELETE_COURSE, { course: payload.course.uuid })
            .then(res => {
              if (Util.isAjaxResOk(res)) {
    ...
    saveCourse: function() {
      ...
      axios
        .post(Constants.API.SAVE_COURSE, {

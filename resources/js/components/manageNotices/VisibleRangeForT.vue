<template>
  <el-form>
    ...
    <el-form-item label="部门" label-width="60px">
      <el-cascader
        style="width:100%;"
        popper-class="tags_cascader"
        ref="tags"
        :props="props"
        :disabled="visibleform.allOran"
        v-model="visibleform.tags"
        @change="changeTag"
      ></el-cascader>
    </el-form-item>
    <el-form-item label="便捷操作" label-width="90px" style="margin-top: 50px;">
      <el-switch
        v-model="visibleform.allOran"
        active-text="所有部门"
        inactive-text
        @change="switchChange"
      ></el-switch>
...
export default {
  ...
  methods: {
    ...
    deleteTag(tag) {
      ...
      this.visibleform.tags = this.visibleform.tags.filter(
        arr => !arr.includes(tag.id)
      );
    },
    confrim() {

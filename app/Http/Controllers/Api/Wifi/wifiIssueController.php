...
class wifiIssueController extends Controller
{
   ...
   public function get_my_issue_info(WifiIssueRequest $request)
   {
      ...
      if ( $infos )
      {
         ...
         // 获取状态流程时间
         $data = [
            ['order_title'=>'报修时间','order_time'=>date('Y-m-d H:i',$infos['created_at'])],
            ['order_title'=>'接单时间','order_time'=>date('Y-m-d H:i',$infos['jiedan_time'])],
            ['order_title'=>'修复时间','order_time'=>date('Y-m-d H:i',   $infos['chulis_time'])],
         ];
